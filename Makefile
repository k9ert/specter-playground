...
build-i18n:
	...
	@cd scenarios/MockUI/i18n && python3 lang_compiler.py compile specter_ui_en.json && mv lang_en.bin ../../../build/flash_image/i18n/
	@if [ -n "$(ADD_LANG)" ]; then \
		for lang in $(shell echo $(ADD_LANG) | tr ',' ' '); do \
			if [ -f scenarios/MockUI/i18n/specter_ui_$$lang.json ]; then \
				echo "  Compiling $$lang..."; \
				cd scenarios/MockUI/i18n && python3 lang_compiler.py compile specter_ui_$$lang.json && mv lang_$$lang.bin ../../../build/flash_image/i18n/ || true; \
			else \
