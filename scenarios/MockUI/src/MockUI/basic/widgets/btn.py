...
from ..utils.ui_consts import BTN_WIDTH, BTN_HEIGHT
...
class Btn(SpecterGuiElement):
    ...
    def __init__(self, parent, icon=None, text=None, size=None,
        ...
        if size is not None:
            w, h = size
            set_size(self, w, h)
            #button shall fill container fully
            set_size(self._btn, lv.pct(100), lv.pct(100))
        else:
            set_size(self, lv.SIZE_CONTENT, lv.SIZE_CONTENT)  # container default-sizes
            set_size(self, lv.SIZE_CONTENT, lv.SIZE_CONTENT)  # container default-sizes
        
        # bubble CLICKED up to Btn so external add_event_cb works
        self._btn.add_flag(lv.obj.FLAG.EVENT_BUBBLE)

        # flex row so content is centered and well spread
